	...
	cin>>tst; // leitura da variável de opções
		...
		if (tst==1){ // início da 1ª opção do menu
				...
				switch (op){ // switch para confirmação de inicialização do jogo ou não
					...
					case 'n':// opção para finalizar o jogo
					...
					while(vida>0){
						...
						if(x==0){
							...
							if(pontos==tam_palavra){
								...
								cout<<"\n A PALAVRA E: "<<palavra;
								cout<<"Deseja retornar ao menu? (S- Sim ou N- Não)";
								char menu2;// variável para menu
								cin>>menu2;// leitura
								switch (menu2){// switch para retornar ao menu ou fechar o jogo
